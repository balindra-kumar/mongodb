...
async function startServer() {
  try {
    ...
    const commentCollection = db.collection(commentTable);
...
// Function to set up API endpoints
function comments(commentCollection) {
  // GET all users
  ...
  app.post('/api/comment', async (req, res) => {
    try {
      const saveData = await commentCollection.insertOne(req.body);
      res.json({ message: 'Post is Submitted!', saveData: saveData });
    } catch (error) {
      console.error('Error saving user:', error);
      res.status(500).json({ message: 'Failed to save user' });
  ...
  app.delete('/api/comment/:id', async (req, res) => {
    try {
      const removedItem = await commentCollection.deleteOne({ _id: new ObjectId(req.params.id) });
      res.json(removedItem);
    } catch (error) {
      console.error('Error deleting user:', error);
      res.status(500).json({ message: 'Failed to delete user', error: error.message });
  ...
  app.patch('/api/comment/:id', async (req, res) => {
    try {
      const postId = new ObjectId(req.params.id);
      const updatePost = await commentCollection.updateOne({ _id: postId }, { $set: req.body });
      res.json({ message: 'User updated successfully', updatePost: updatePost });
    } catch (error) {
      console.error('Error updating user:', error);
      res.status(500).json({ message: 'Failed to update user', error: error.message });
  ...
  app.get('/api/comment/:id', async (req, res) => {
    try {
      const singlePostId = new ObjectId(req.params.id);
      const fetchSinglePost = await commentCollection.findOne({ _id: singlePostId });
      res.json({ message: 'User fetched successfully', user: fetchSinglePost });
    } catch (error) {
      console.error('Error fetching single user:', error);
      res.status(500).json({ message: 'Failed to fetch user', error: error.message });
